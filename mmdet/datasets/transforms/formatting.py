...
@TRANSFORMS.register_module()
class PackPlumeInputs(BaseTransform):
    mapping_table = {
        'gt_bboxes': 'bboxes',
        'gt_bboxes_labels': 'labels',
        'gt_bboxes_emissions': 'bboxes_emissions',
        'gt_masks': 'masks',
    }

    def __init__(self,
                 meta_keys=('img_id', 'plume_path', 'wind_path',  'ori_shape', 'img_shape',
                            'scale_factor', 'flip', 'flip_direction')):
        self.meta_keys = meta_keys
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'wind' in results:
            ...
            if not wind.flags.c_contiguous:
                wind = np.ascontiguousarray(wind.transpose(2, 0, 1))
                wind = to_tensor(wind)
            else:
                wind = to_tensor(wind).permute(2, 0, 1).contiguous()

            packed_results['scalars'] = wind
